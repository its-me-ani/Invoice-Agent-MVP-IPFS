...
const Menu: React.FC<{
  ...
  const doGenerateCSV = async (filename?: string) => {
    try {
      ...
      const csvFilename = filename || selectedFile || "Invoice";
  ...
  const doExportAllSheetsAsPDF = async (filename?: string) => {
    try {
      ...
      const pdfFilename =
        filename || `${selectedFile}_all_sheets` || "all_rent_receipts";
          ...
          try {
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                await Share.share({
                  title: `${pdfFilename}.pdf`,
                  text: `Combined PDF with ${sheetsData.length} sheets generated successfully`,
                  url: tempFile.uri,
      ...
      try {
        ...
        if (isPlatform("ios")) {
          try {
              ...
              reader.onloadend = async () => {
                ...
                try {
                  ...
                  await Share.share({
                    ...
                    text: `Please find the attached invoice with ID: ${invoiceIdentifier}`,
                    url: tempFile.uri,
            ...
            setToastMessage("Failed to prepare cash receipt for email.");
            setShowToast1(true);
          }
        } else {
          ...
          await EmailComposer.open({
            ...
            cc: [],
            bcc: [],
            body: `Please find the attached invoice with ID: ${invoiceIdentifier}`,
            attachments: [{ type: "absolute", path: filePath, name: filename }],
            subject: "Here is your Cash Receipt",
            isHtml: true,
          });
  ...
  const generateInvoiceFilename = () => {
    ...
    return `receipt-${year}${month}${day}-${hours}${minutes}${seconds}`;
  };
