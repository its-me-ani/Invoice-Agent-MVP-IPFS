...
const SocialCalcPage: React.FC = () => {
  ...
  const [showCloudAgentPanel, setShowCloudAgentPanel] = useState(false);
    ...
    try {
      ...
      if (ipfsTempContentStr) {
        ...
        try {
          ...
          console.log("=== DEBUG: SocialCalcPage IPFS Load ===");
          console.log("tempInvoice keys:", Object.keys(tempInvoice));
          console.log("tempInvoice.templateId:", tempInvoice.templateId);
          console.log("tempInvoice.content type:", typeof tempInvoice.content);
          ...
          if (typeof content === 'string') {
            console.log("DEBUG: content is string, length:", content.length);
            try {
          ...
          if (typeof content === 'object' && content !== null) {
            console.log("DEBUG: content is object, keys:", Object.keys(content));
            console.log("DEBUG: content.msc defined:", content.msc !== undefined);
            console.log("DEBUG: content.footer defined:", content.footer !== undefined);
            console.log("DEBUG: content.footers defined:", content.footers !== undefined);
            console.log("DEBUG: content.appMapping defined:", content.appMapping !== undefined);
          }

          if (typeof content === 'object' && content.msc !== undefined) {
            storedAppMapping = content.appMapping;
            storedFooter = content.footer || content.footers;
            content = content.msc;
            console.log("DEBUG: Extracted msc, type:", typeof content);
            console.log("DEBUG: storedAppMapping:", storedAppMapping ? Object.keys(storedAppMapping) : null);
            console.log("DEBUG: storedFooter:", storedFooter ? (Array.isArray(storedFooter) ? `array[${storedFooter.length}]` : typeof storedFooter) : null);
            if (typeof content === 'object') {
              console.log("DEBUG: msc keys:", Object.keys(content));
              content = JSON.stringify(content);
            }
            console.log("DEBUG: contentToLoad (first 500 chars):", typeof content === 'string' ? content.substring(0, 500) : content);
          } else if (typeof content === 'object') {
            console.log("DEBUG: content is object but NO msc key, stringifying");
            content = JSON.stringify(content);
          }
          ...
          console.log("DEBUG: Final contentToLoad type:", typeof contentToLoad, "length:", contentToLoad?.length);
          console.log("DEBUG: templateId:", templateId);
          console.log("DEBUG: footerIndex:", footerIndex);

          if (storedAppMapping) {
            templateData = {
              appMapping: storedAppMapping,
              footers: storedFooter ? (Array.isArray(storedFooter) ? storedFooter : [storedFooter]) : [],
              msc: { currentid: 'sheet1' }
            };
            console.log("DEBUG: Built templateData, footers count:", templateData.footers.length);
          }
          ...
          if (!templateData) {
            ...
            console.log("DEBUG: Using empty fallback templateData");
          }

          console.log("=== DEBUG: IPFS Load Complete ===");
          console.log("DEBUG: Final templateData footers count:", templateData?.footers?.length);
          console.log("DEBUG: Final contentToLoad exists:", !!contentToLoad);
      ...
      setTimeout(() => {
        ...
        try {
          ...
          // Detect if contentToLoad is JSON format (from agent reports/templates)
          // vs native SocialCalc save format (from IPFS cloud saves).
          // viewFile/WorkBookControlInsertWorkbook only handles native format,
          // so JSON content must always go through initializeApp/WorkBookControlLoad.
          const isJsonContent = contentToLoad && contentToLoad.trimStart().startsWith('{');

          if (currentControl && currentControl.numsheets > 0 && !isJsonContent) {
            // SocialCalc is initialized and content is native format, use viewFile
            console.log(
              "✅ initializeApp: SocialCalc already initialized, using viewFile"
            );
            AppGeneral.viewFile(fileToLoad, contentToLoad);
          } else {
            // Either SocialCalc not initialized, or content is JSON format
            // which requires a fresh WorkBookControlLoad
  ...
  return (
    ...
    >
        ...
        <IonToolbar className="invoice-toolbar" style={{ paddingTop: `max(env(safe-area-inset-top, 0px), 24px)`, minHeight: '56px' }}>
          ...
          >
            <IonIcon
              ...
              style={{ fontSize: "22px", color: "#6366f1", cursor: "pointer", marginRight: "12px" }}
              onClick={() => setShowCloudAgentPanel(true)}
      ...
      >
        ...
        <IonAlert
          ...
          buttons={[
            ...
            },
            {
              text: "Save",
              handler: (data) => {
                console.log("DEBUG Save handler raw data:", data);
                if (data.invoiceName && data.invoiceName.trim()) {
                  const invoiceName = data.invoiceName.trim();
                  console.log("DEBUG Save handler: invoiceName =", invoiceName, "fileName =", fileName, "isNewTemplateFileName =", isNewTemplateFileName);
                  ...
                  (async () => {
                    ...
                    if (fileName === "invoice" || isNewTemplateFileName) {
                      ...
                      console.log("DEBUG Save handler: generated filename =", filename);

                      try {
                        const exists = await localTemplateService.invoiceExists(filename);
                        console.log("DEBUG Save handler: exists =", exists);
                        if (exists) {
                      ...
                      console.log("DEBUG Save handler: setting saveInvoiceName and setShowSaveInvoiceDialog(false)");
        ...
        {/* Cloud Agent Panel */}
        <CloudAgentPanel
          isOpen={showCloudAgentPanel}
          onClose={() => setShowCloudAgentPanel(false)}
          userId={userId}
          onReportLoaded={(report) => {
            try {
              if (report && report.content) {
                // Load report directly into SocialCalc without navigation
                let content = report.content;
                let mscData: string;
                let reportTemplateData: any = null;

                // Extract MSC and template data from report content
                if (typeof content === 'object' && content.msc !== undefined) {
                  const storedAppMapping = content.appMapping;
                  const storedFooter = content.footer || content.footers;
                  const msc = content.msc;
                  mscData = typeof msc === 'object' ? JSON.stringify(msc) : msc;
                  ...
                  try { mscData = decodeURIComponent(content); } catch (e) { mscData = content; }
                } else {
                  mscData = JSON.stringify(content);
                }

                // Update template data in context
                if (reportTemplateData) {
                  updateActiveTemplateData(reportTemplateData, report.templateId || activeTemplateId || billType);
                }

                // Load into SocialCalc directly
                AppGeneral.initializeApp(mscData);

                // Activate footer after loading
                const footerIndex = report.billType || 1;
                setTimeout(() => {
                  activateFooter(footerIndex);
                }, 500);

                setToastMessage("Report loaded successfully!");
                setToastColor("success");
                setShowToast(true);
              }
            } catch (err) {
              console.error("Failed to load cloud agent report into editor:", err);
              setToastMessage("Failed to load report into editor");
              setToastColor("danger");
              setShowToast(true);
            }
          }}
