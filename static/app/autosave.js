...
Aspiring.AutoSave.TimerExpiry = function () {
    ...
    if (SocialCalc.GetCurrentWorkBookControl().workbook.spreadsheet.editor.state == "start") {
        ...
        // Send appMapping and metadata as separate fields (not embedded in MSC data)
        if (SocialCalc.AppMapping) {
            message.appMapping = JSON.stringify(SocialCalc.AppMapping);
        }
        if (SocialCalc.FileMetadata) {
            message.metadata = JSON.stringify(SocialCalc.FileMetadata);
        }
