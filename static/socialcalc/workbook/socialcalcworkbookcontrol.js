...
SocialCalc.WorkBookControlRenameSheetSubmit = function () {
	...
	// prevent a sheetname prefixed with sheet
	if (smallname.indexOf("sheet") == 0) {
		alert("sheetname cannot start with 'sheet'");
		return;
	}
...
SocialCalc.SheetBarSheetButton = function (id, name, parentdiv, params, functions, img) {
	...
	if (!img) {
		...
		imgele.id = id + "-img";
		imgele.src = SocialCalc.Constants.defaultImagePrefix + "menu-dropdown.svg";
...
SocialCalc.SpinnerWaitCreate = function () {
	...
	main.innerHTML = '<img src="static/images/spinner.svg" alt="Loading..." />';
...
SocialCalc.Callbacks.GetCellEditableAttribs = function (attribs, coord) {
	var control = SocialCalc.GetCurrentWorkBookControl();
	...
	if (!(SocialCalc.EditableCells.allow)) {
		// by default all cells are editable
		attribs["ceditable"] = { def: true, val: "" };
		attribs["cconstraints"] = { def: true, val: "" };
		return;
	} else if (SocialCalc.EditableCells.cells[cellname]) {
		attribs["ceditable"] = { def: false, val: "yes" };
	} else {
		attribs["ceditable"] = { def: false, val: "no" };
	}
	/*
	if (SocialCalc.EditableCells.constraints[cellname]) {
	attribs["cconstraints"] = {def: false, val: SocialCalc.EditableCells.constraints[cellname]};	
	} else {
	attribs["cconstraints"] = {def: true, val: ""};	
	} 
	*/
...
SocialCalc.Callbacks.SetCellEditableAttribs = function (coord, attribs, range) {
	var control = SocialCalc.GetCurrentWorkBookControl();
	var editor = control.workbook.spreadsheet.editor;
	var sheetname = editor.workingvalues.currentsheet

	if (range) {
		var lis = range.split(":");
		var obj1 = SocialCalc.coordToCr(lis[0]);
		var obj2 = SocialCalc.coordToCr(lis[1]);
		var i = 0;
		var j = 0;
		for (i = obj1.col; i <= obj2.col; i++) {
			for (j = obj1.row; j <= obj2.row; j++) {
				var cellname = sheetname + "!" + SocialCalc.crToCoord(i, j)
				if (!attribs["ceditable"].def && attribs["ceditable"].val == "yes") {
					SocialCalc.EditableCells.allow = true
					SocialCalc.EditableCells.cells[cellname] = true;
				}
				if (!attribs["ceditable"].def && attribs["ceditable"].val == "no") {
					delete SocialCalc.EditableCells.cells[cellname];
					if (!SocialCalc.EditableCells.cells.length) {
						SocialCalc.EditableCells.allow = false
					}
				}
				/*
				if (!attribs["cconstraints"].def) {
					SocialCalc.EditableCells.constraints[cellname] = attribs["cconstraints"].val;
					SocialCalc.EditableCells.allow = true
				}
				*/
			}
		...
		var cellname = sheetname + "!" + editor.ecell.coord;
	...
	if (constraint != null) {
		switch (constraint[0]) {
			case "numeric":
				...
				//alert(val)
				if (isNaN(val)) {
					//Aspiring.showPopUp("please input a number");
					alert("please input a number");
					return false;
				}
				if (val < low) {
					//Aspiring.showPopUp(text +" must be at least "+low);
					alert(text + " must be at least " + low);
					return false;
				}
				if (val > high) {
					//Aspiring.showPopUp(text + " must be atmost "+high);
