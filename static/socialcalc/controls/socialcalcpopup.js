   ...
   if (spcdata.mainele && spcdata.mainele.firstChild) {
      if (spcdata.value) {
         ...
         if (spca.backgroundImageDefault) {
            ...
            img = "url(" + sp.imagePrefix + "defaultcolor.svg)";
            pos = "left top";
         }
      }
...
SocialCalc.Popup.Types.ColorChooser.CreateGrid = function (type, id) {
   ...
   ele.innerHTML = '<table cellspacing="0" cellpadding="0"><tr>' +
      '<td style="width:17px;background-color:#FFF;background-image:url(' + sp.imagePrefix + 'defaultcolor.svg);height:16px;font-size:10px;cursor:pointer;" title="' + SPLoc("Default") + '">&nbsp;</td>' +
...
SocialCalc.Popup.Types.ColorChooser.SetColors = function (id) {
   ...
   for (row = 0; row < 16; row++) {
      for (col = 0; col < 5; col++) {
         ...
         if (grid["selectedrow" + col] == row) {
            g.ele.style.backgroundImage = "url(" + sp.imagePrefix + "chooserarrow.svg)";
         }
         else {
   ...
   if (!spcdata.value) { // default
      ...
      grid.msg.style.backgroundImage = "url(" + sp.imagePrefix + "defaultcolor.svg)";
