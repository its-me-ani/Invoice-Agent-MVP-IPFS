...
SocialCalc.ExecuteSheetCommand = function (sheet, cmd, saveundo) {
   ...
   switch (cmd1) {
      ...
      case "set":
         ...
         if (what == "sheet") {
         ...
         else if (/^\d+(:\d+)?$/.test(what)) { // row attributes
            var r1, r2, r;
            sheet.renderneeded = true;
            pos = what.indexOf(":");
            if (pos >= 0) {
               r1 = what.substring(0, pos) - 0;
               r2 = what.substring(pos + 1) - 0;
            }
            else {
               r1 = what - 0;
               r2 = r1;
            ...
            for (r = r1; r <= r2; r++) {
               if (attrib == "height") {
                  if (saveundo) changes.AddUndo("set " + r + " height", sheet.rowattribs.height[r]);
                  if (rest.length > 0) {
                     sheet.rowattribs.height[r] = rest - 0;
                  }
                  else {
                     delete sheet.rowattribs.height[r];
                  }
               }
               else if (attrib == "hide") {
                  if (saveundo) changes.AddUndo("set " + r + " hide", sheet.rowattribs.hide[r]);
                  if (rest.length > 0) {
                     sheet.rowattribs.hide[r] = rest;
                  }
                  else {
                     delete sheet.rowattribs.hide[r];
                  }
               }
               else {
...
SocialCalc.RenderContext = function (sheetobj) {
   ...
   this.highlightTypes = // attributes to change when highlit
   {
      ...
      cursorinsertup: { style: "color:#FFF;backgroundColor:#A6A6A6;backgroundRepeat:repeat-x;backgroundPosition:top left;backgroundImage:url(" + scc.defaultImagePrefix + "cursorinsertup.svg);", className: scc.defaultHighlightTypeCursorClass },
      cursorinsertleft: { style: "color:#FFF;backgroundColor:#A6A6A6;backgroundRepeat:repeat-y;backgroundPosition:top left;backgroundImage:url(" + scc.defaultImagePrefix + "cursorinsertleft.svg);", className: scc.defaultHighlightTypeCursorClass },
      range2: { style: "color:#000;backgroundColor:#FFF;backgroundImage:url(" + scc.defaultImagePrefix + "range2.svg);", className: "" }
   }
...
SocialCalc.RenderContext.prototype.RenderCell = function (rownum, colnum, rowpane, colpane, noElement, linkstyle) { return SocialCalc.RenderCell(this, rownum, colnum, rowpane, colpane, noElement, linkstyle); };
