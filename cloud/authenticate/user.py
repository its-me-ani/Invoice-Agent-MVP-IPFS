...
def _init_user_directory():
    ...
    storage.ensureHomeRootExists()

    # Then create the users directory if it doesn't exist
    if not storage.existsItem(userdirpath):
        storage.createDir(userdirpath)
        print(f"Created user directory: {userdirpath}")
